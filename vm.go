...
type VM struct {
	memory     *Memory
	registers  []gpRegister // general-purpose registers
...
func NewVM() *VM {
	...
	// Interrupt registers.
	for creg := CREG_INT_FIRST; creg < CREG_INT_LAST+1; creg++ {
		vm.creg[creg] = 0xffffffff
	}
	vm.creg[CREG_INT_CONTROL] = 0 // Maskable interrupts disabled.
...
func (vm *VM) fetchPendingInterrupt() *int {
	...
	// (faults).
	if vm.creg[CREG_INT_CONTROL]&1 == 0 {
...
func (vm *VM) processInterruptQueue() error {
	...
	for _, register := range vm.registers {
	...
	vm.sp.value = tmpSp
	vm.pc.value = uint32(vm.creg[CREG_INT_FIRST+(*i&0xf)])
	return nil
}
